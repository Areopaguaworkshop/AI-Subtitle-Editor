...
def _(dspy, parse_subtitle, spacy):
    def rewrite_text(file_path):
        """Rewrites text from oral to written form using Gemini API.
        """
        """
        os.environ["GEMINI_API_KEY"] = "GEMENI_API_KEY"
        lm = dspy.LM("gemini/gemini-1.5-flash")
        """
        # lm=dspy.OllamaLocal(model='deepseek-r1:8b', base_url= "http://localhost:11434", max_tokens= 1000, timeout_s=500)
        lm=dspy.OllamaLocal(model='deepseek-r1:8b', base_url= "http://localhost:11434", max_tokens= 15000, timeout_s=600)
        ...
        for sentence in sentences:
            ...
            # rewritten_sentences.append(response)
